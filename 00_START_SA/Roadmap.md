...
changed_by: Runner
summary: (v0.3:新增 §7 工作波次與並行關係——未結項目全盤點(軌 A 5 / 軌 B 12 / 軌 C 三區塊)、波 0~波 5 觸發條件式排程、SA 與 Runner 並行關係;維持不排日期)(v0.2:SprintReview Backlog 對帳增補——分批表納 M04-F05 / M08-F01 / M08-F02,新增批 9;M05-F04 標 PLANNED 不排批;F 35 → 39)SA 作業路線圖——五階段 × 三並行軌的全局路線、逐階段入場/出場條件、35 F 分批展開順序、阻塞依賴矩陣與退路。只排順序與依賴,不排日期。屬 SA 內部工具,非交付物(AI_Rules §5 條 12)。
---
# SA 作業路線圖(Roadmap)
...
### R6 · `21_AddModule.md` v2.14 DoD 第 5 條與 §1 編號 5 相斥(可能性:必然,影響:中)

**衝突**:v2.14 新增的 DoD 第 5 條要求「對新 F 模組目錄下所有相對連結逐一驗證目標實體存在…**失聯 > 0 時先修正連結再出本報告**」;但同檔 §1 編號 5 明定「**W99 以外的 W## 節點檔,本階段一律不建立**」,且 L3 範本的業務流程區塊列為「不動的內容」。

範本 `F00_L3_Workflow.md` 內建兩條 `click ... "./nodes/M##-F##-W01_xxx.md"` / `W02` 佔位連結,指向本階段依規定**不得建立**的檔案。兩條同時遵守做不到 —— 要命中就得建 W01/W02(違反編號 5),要移除佔位行就得動業務內容(亦違反編號 5)。

**實測**:2026-09-01 建置 SYS01 + SYS03 後,29 個 F 共 87 條相對連結,命中 29(每 F 一條 `04_FuncMap.md` 回鏈)、「失聯」58(每 F 兩條 W01/W02 佔位)。

**處置(待 SA 拍板)**:Runner 判讀 DoD 5 的射程應為「**本階段實際產出的結構性連結**」,業務佔位連結於階段 3 建 W## 時自然消解。此為解讀而非套件明文,故列 §7.2 軌 A 待 SA 認可。

**上游**:此為本專案發現的**第三個套件內部矛盾**(前兩個:範本目錄大小寫 R2、`04_FuncMap.md` 維護權歸屬)。SA 認可解讀後,應併同回報上游。

### R7 · `03_Structure.md` 現況與 DEC-012 相斥(可能性:已發生,影響:低)

**衝突**:`03_Structure.md` 現行結構為 `## SYS01 → ### SS01_完整版 → #### M07 → ##### F01`,建了 SS 標題層;但 **DEC-012 明定 SYS01 不建 SS 標題層**(SYS01 的 SS 歸屬逐 F 而定,同一 M 底下的 F 分屬不同 SS,M 掛不進單一 SS)。

**成因**:2026-09-15 產出 `M07-F01` 時 DEC-012 尚未落檔(該決策 09-01 作成、09-16 始補入 `00_Glossary.md`),故照 `21_AddModule.md` 預設路徑建了 SS 層。

**處置(待 SA 拍板)**:Runner 建議**改 `03_Structure.md`** —— 移除 SYS01 的 SS 標題層,M 升 `###`、F 升 `####`;SYS02 因 M 層乾淨對應 SS03 / SS04,**照建** SS 標題層。反向(改 DEC-012)等於要在 SYS01 硬塞一個容不下的層級。

**影響範圍**:僅標題層級,不動任何節點登記列與實體路徑。
...
| 一個工作波次完成或觸發 | §7.3 波次表狀態;§7.2 對應軌的項目列 | Runner |

**不在本檔維護的**:當前流程位置、待 SA 回應問題的內容本體、各 F 的規格內容、TBD 本體。**實際週次亦不在此維護**——待 SA 與工程團隊議定後,依 §7.5 於 §7 加註。分別歸 `00_Status.md`、`00_Glossary.md`、各規格檔案。
...
> 本節回答「**每一項任務在什麼時候處理**」。
> **不排日期、不排週次** —— 12 項工程端回覆的時程 SA 與 Runner 皆不可控,標日期等於標假資料。改以**觸發條件**排程:每一波寫明「由什麼事件啟動」與「完成判準」。
> SA 於看過本表並與工程團隊討論後,方能拍板實際週次;在此之前本節只定順序與並行關係。

### 7.1 排程的核心原則

**把「只有 SA 能做的」與「Runner 能做的」分開跑,兩邊不互等。**

SA 的時間卡在兩件事:**拍板**(短,以分鐘計)與 **D1.5 審閱**(長,逐 W 節點看)。Runner 的產出不得排在 SA 的審閱之後 —— 否則等待期空轉。
...
| 項目 | 擋住什麼 | 成本 |
|---|---|---|
| **C12** `M08 對外服務` 命名與歸屬 | SS05 + M08 + 其下 2 F 無法建立 | 🟢 零成本(純命名判斷) |
| **DoD-5 射程解讀** | 階段 2 收尾驗收無法通過 | 🟢 零成本(見 §5 R6) |
| **`03_Structure.md` SS 標題層衝突** | 同上 | 🟢 零成本(見 §5 R7) |
| C09 系統管理員是否綁 GID | 批 5(M05 三個 F) | 🟡 需查證 |
| O02 調查版是否需離線 | SS02 技術架構;批 4 的 M03 系列 | 🟡 需查證 |

> 📌 **C09 與 O02 不必 SA 獨力承擔** —— 兩者即 **E6 / E11**,工程端同樣能答。建議一併併入軌 B 發包,誰先給答案都算解。

**軌 B · 待工程端回(12 項)**

| 🔴 關鍵路徑 | 擋住什麼 |
|---|---|
| **E14** 稽核紀錄的資料結構、寫入與效能 | `M07-F01` 無法走到 D2 收斂,批 1 無法閉環 |
| **E15** 植栽狀態異動有無歷史保存 | `M04-F04` / `M04-F05`;Backlog 第 4、5 條 AC |

其餘 10 項:E2 / E3(擋批 5)、E6 / E11(= C09 / O02)、E12(擋 `SYS02-M02-F04`)、E13(部署型態,弱)、E16、E17(擋 `M08-F02`)、E18、E19。

**軌 C · 規格產出**

| 區塊 | 未竟內容 |
|---|---|
| 階段 2 未竟 | SYS02(2 M / 6 F)、`M04-F05`、`M08`(卡 C12)、`03_Structure.md` 定稿、`04_FuncMap.md` 定稿 |
| 階段 3 | 批 1 另一半 `M01-F03`;批 2 ~ 批 9(37 F 未做) |
| 階段 4 | `M07-F01` 的 D1.5 審閱 → D2 收斂 |

**其他掛帳**:`00_START_SA/AGENT_TASK_範本目錄大小寫.md` 仍為 `DRAFT`,待 SA 決定是否轉交上游。

### 7.3 波次表

| 波 | 誰做 | 啟動觸發 | 內容 | 完成判準 |
|---|---|---|---|---|
| **波 0** | **SA** | 立即 | 清三項 🟢 零成本決策:**C12** / **DoD-5 射程** / **`03_Structure.md` 層級** | 三項皆有明確答覆;Runner 落 DEC 至 `00_Glossary.md` |
| **波 1** | **SA** | 與波 0 同時 | 將 `EngRegister.md` 待回 12 項發給工程端,**E14 / E15 標優先** | 清單已送出(一次搬運) |
| **波 2** | **Runner** | 波 0 完成 | **階段 2 收尾**:SYS02 結構、`M04-F05`、`M08`(C12 通過則建)、兩本中樞表定稿轉 `ACTIVE` | §2 階段 2 出場條件逐條通過 |
| **波 3** | **SA** | 立即(不等波 2) | **`M07-F01` D1.5 審閱**:W01~W06 逐節點審 | 節點狀態轉 `S3_READY_SYNC` |
| **波 4** | **Runner** | 波 2 完成 | 批 1 另一半 **`M01-F03` 廠商群組管理** R2 Part 2 + D1 | §2 階段 3 出場條件逐條通過 |
| **波 5+** | 依回覆分流 | 見下 | — | — |
...
| 觸發 | 動作 |
|---|---|
| **E14 回填** | `M07-F01` 走 D2 收斂,批 1 閉環 |
| **E15 回填** | 解鎖 `M04-F04` / `M04-F05`;Backlog 第 4、5 條 AC 可寫 |
| **E2 / E3 / E6 回填** | 解鎖批 5(M05 三個 F + `M06-F01`) |
| **E17 回填** | 解鎖 `M08-F02` 外部照片審核機制 |
| **皆未回** | **不停工** —— 照 §3 往下做**批 2 `M04-F01` 維養紀錄**(不受任何未回項阻塞) |

### 7.4 並行關係

| 段 | SA | Runner |
|---|---|---|
| 一 | 波 0 拍板 → 波 1 發包 | (等波 0) |
| 二 | **波 3 審閱 `M07-F01`** | **波 2 階段 2 收尾** |
| 三 | (等工程端回覆) | 波 4 `M01-F03` |
| 四 | 依回覆拍板 | 波 5 分流 |

**設計意圖**:SA 的總投入集中於**前段拍板與發包**、**中段審閱**兩處;工程端 12 項的等待期由波 2 與波 4 完全吃掉,不空轉。

### 7.5 本節的維護

| 何時更新 | 更新什麼 |
|---|---|
| 一波完成 | 該波標完成;若觸發下一波,更新啟動狀態 |
| 軌 A 任一項拍板 | §7.2 軌 A 該列移除 |
| 軌 B 任一項回填 | §7.2 軌 B 該列移除;§7.3 波 5 分流表對應列標已觸發 |
| SA 與工程團隊議定實際週次 | **於本節加註週次欄**,不改觸發條件本身 |
